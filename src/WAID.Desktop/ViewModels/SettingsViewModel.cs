...
public sealed class SettingsViewModel : ViewModelBase
{
    ...
    private async Task LoadAsync(){try{var s=await _repository.GetAsync(CancellationToken.None);RunScansAtStartup=s.RunScansAtStartup;EnableAiAnalysis=s.EnableAiAnalysis;AllowTelemetry=s.AllowTelemetry;Theme=s.Theme;}catch(Exception ex){Status=$"Could not load settings: {ex.Message}";}}
    private async Task SaveAsync(){try{await _repository.SaveAsync(new ApplicationSettings{RunScansAtStartup=RunScansAtStartup,EnableAiAnalysis=EnableAiAnalysis,AllowTelemetry=AllowTelemetry,Theme=Theme},CancellationToken.None);Status="Settings saved";}catch(Exception ex){Status=$"Could not save settings: {ex.Message}";}}
}
