...
public sealed class SqliteScanScheduleRepository(WaidDatabase database) : IScanScheduleRepository
{
    ...
    public async Task SaveHistoryAsync(ScheduledScanHistory history, CancellationToken token) { await using var connection=database.OpenConnection(); await using var command=connection.CreateCommand(); command.CommandText="INSERT INTO scheduled_scan_history(id,evaluated_utc,outcome,history_json) VALUES($id,$time,$outcome,$json)"; command.Parameters.AddWithValue("$id",history.Id.ToString()); command.Parameters.AddWithValue("$time",history.EvaluatedAtUtc.ToString("O")); command.Parameters.AddWithValue("$outcome",(int)history.Outcome); command.Parameters.AddWithValue("$json",JsonSerializer.Serialize(history)); await command.ExecuteNonQueryAsync(token).ConfigureAwait(false); }
    public async Task<IReadOnlyList<ScheduledScanHistory>> GetHistoryAsync(int count,CancellationToken token) { if(count is < 1 or > 500) throw new ArgumentOutOfRangeException(nameof(count)); var items=new List<ScheduledScanHistory>(); await using var connection=database.OpenConnection(); await using var command=connection.CreateCommand(); command.CommandText="SELECT history_json FROM scheduled_scan_history ORDER BY evaluated_utc DESC,id DESC LIMIT $count"; command.Parameters.AddWithValue("$count",count); await using var reader=await command.ExecuteReaderAsync(token).ConfigureAwait(false); while(await reader.ReadAsync(token).ConfigureAwait(false)) items.Add(JsonSerializer.Deserialize<ScheduledScanHistory>(reader.GetString(0))??throw new InvalidDataException("Invalid scheduled scan history.")); return items; }
}
