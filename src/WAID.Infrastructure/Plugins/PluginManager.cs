...
public sealed class PluginManager(string directory,Version hostVersion,PluginSecurityPolicy policy,PluginCatalog catalog,IPluginInventoryRepository inventory,IAuditTrailService audit,TimeProvider time)
{
 ...
 {
  var item=catalog.Find(pluginId)??throw new InvalidOperationException("Plugin inventory item was not found.");PluginLoader.SetDisabled(directory,pluginId,!enabled);var state=PluginState.RestartRequired;var detail=enabled?"Enable requested; restart WAID to recertify and load.":"Disabled; restart WAID to unload registered services.";var updated=item with{State=state,Detail=detail};catalog.Replace(catalog.Items.Where(x=>!x.Id.Equals(pluginId,StringComparison.OrdinalIgnoreCase)).Append(updated));await Save(updated,token);await Audit(pluginId,enabled?"PluginEnable":"PluginDisable",AuditResult.Succeeded,detail,token);return new(true,enabled?"PLUGIN_ENABLE_PENDING":"PLUGIN_DISABLED",detail,true);
 }
