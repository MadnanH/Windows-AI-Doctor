...
public sealed class DiagnosticReportExporter(string outputDirectory, IPerformanceTelemetry? performance = null) : IDiagnosticReportExporter
{
    ...
    private static string RenderHtml(DiagnosticReportData report)
    {
        ...
        var causes = report.Diagnosis?.RootCauses.Select(c => $"<li><strong>{E(c.ExplanationDetail.ProblemStatement)}</strong> - {c.Confidence}% ({E(c.ExplanationDetail.Calibration.Band)}): {E(c.ExplanationDetail.Rationale)}<br><strong>Impact:</strong> {E(c.ExplanationDetail.Impact)}<br><strong>Urgency:</strong> {E(c.ExplanationDetail.Urgency)}<br><strong>Next step:</strong> {E(c.ExplanationDetail.NextStep)}<br><strong>Change:</strong> {E(c.ExplanationDetail.ChangeOverTime)}<br><strong>Alternatives:</strong> {E(string.Join("; ", c.ExplanationDetail.Alternatives.Select(a => a.Cause)))}<br><strong>Counter-evidence:</strong> {E(string.Join("; ", c.ExplanationDetail.CounterEvidence.Select(e => e.Title)))}</li>") ?? [];
