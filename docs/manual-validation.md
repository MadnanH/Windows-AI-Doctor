...
The JSON reports are consumed directly by the Prompt 40 release-candidate harness. Preserve them with the protected release-validation record. A scenario is accepted only when every check is marked `pass`; the script exits with code 1 when any check fails.
