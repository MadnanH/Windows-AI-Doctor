...
`ConfigureServices` may register implementations of `ISystemScanner`, `IRepairAction`, or `IAiAnalyzer`. Keep registration side-effect free; initialize hardware or network resources only when the service is used. Copy the plugin and its private dependencies into the application `Plugins` directory.
