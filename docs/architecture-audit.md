...
| SQLite persistence | Complete for schema 16 | All repositories, versioned configuration, scanner execution state, driver, boot, update, storage, security posture, chat, and network diagnostic history use `WaidDatabase`; Settings exposes health and recovery | Repository, migration, configuration, scanner provenance, diagnostic history, chat/network lifecycle, corruption, concurrency, backup, and recovery tests | Continue adding repositories as reserved schema areas gain behavior |
