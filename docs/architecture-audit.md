...
| SQLite persistence | Complete for schema 14 | All repositories, versioned configuration, scanner execution state, driver, boot, update, storage, and security posture history use `WaidDatabase`; Settings exposes health and recovery | Repository, migration, configuration, scanner provenance, diagnostic history, corruption, concurrency, backup, and recovery tests | Continue adding repositories as reserved schema areas gain behavior |
