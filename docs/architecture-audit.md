...
| SQLite persistence | Complete for schema 9 | All repositories, versioned configuration and scanner execution state, and maintenance operations use `WaidDatabase`; Settings exposes health and recovery | Repository, migration, configuration, scanner provenance, corruption, concurrency, backup, and recovery tests | Continue adding repositories as reserved schema areas gain behavior |
