...
| SQLite persistence | Complete for schema 10 | All repositories, versioned configuration, scanner execution state, and driver analysis history use `WaidDatabase`; Settings exposes health and recovery | Repository, migration, configuration, scanner provenance, driver history, corruption, concurrency, backup, and recovery tests | Continue adding repositories as reserved schema areas gain behavior |
